...
metadata {
	definition (name: "POPPTRV - modificat", namespace: "", author: "Tom Philip") {
		capability "Actuator"
		capability "Sensor"
  ...
  // Tried to mirror Nest http://thingsthataresmart.wiki/index.php?title=Nest_Manager#Nest_Thermostat
    ...
    tiles(scale: 2) {
		multiAttributeTile(name:"richtemp", type:"thermostat", width:5, height:3) {
        ...
        // Reglare temperatura manual
			tileAttribute("device.nextHeatingSetpoint", key: "VALUE_CONTROL") {
				attributeState("VALUE_UP", action: "temperatureUp", backgroundColor:"#9370db")
				attributeState("VALUE_DOWN", action: "temperatureDown", backgroundColor:"#9370db")
			}
            
		// Temperatura afisata intre butoanele sus si jos
			...
			tileAttribute("device.nextHeatingSetpoint", key: "HEATING_SETPOINT") {
					attributeState("default", label:'${currentValue}', unit:"°C")
			}
		}

			valueTile("battery", "device.battery", inactiveLabel: false, height: 2, width: 6, decoration: "flat") {
			state "battery", label:'${currentValue}% battery', unit:""
		}


		// Afisare stare termostat: inchis / deschis in meniul principal 
		standardTile("switcher", "device.switch", height: 2, width: 6, decoration: "flat") {
			state("off", action:"on", label: "Inchis", backgroundColor:"#8ebfed")
			state("on", action:"off", label: "Deschis", backgroundColor:"#e5b960")
            state "default", label: '${currentValue}', action:"refresh.refresh", icon:"st.secondary.refresh"
		}
		
		// Menu principal
		main "richtemp"  
		details(["richtemp", "switcher", "battery", "refreshTile"])

	}

	preferences {
		input "wakeUpIntervalInMins", "number", title: "Wake Up Interval (min). Default 1min.", description: "Wakes up and send\receives new temperature setting", range: "1..30", displayDuringSetup: true
		input "quickOnTemperature", "number", title: "Quick On Temperature. Default 28°C.", description: "Quickly turn on the radiator to this temperature", range: "5..82", displayDuringSetup: false
...
//	WakeUpINterval 60 (1min)

//	defaultWakeUpIntervalSeconds: 60, maximumWakeUpIntervalSeconds: 1800 (30 mins),
...
def setHeatingSetpoint(Double degrees) {
	...
	sendEvent(name:"nextHeatingSetpoint", value: degrees.round(3), unit: getTemperatureScale())
...
def setHeatingSetpointCommand(Double degrees) {
	...
	def precision = state.precision ?: 3
/*
AICI
*/
...
def installed() {
    ...
    // WakeUpNotifications to the hub every 1 min intially
    zwave.wakeUpV1.wakeUpIntervalSet(seconds:60, nodeid:zwaveHubNodeId).format()
	], 1000)
}
...
def configure() {
	log.debug("configure")
	def wakeUpEvery = (wakeUpIntervalInMins ?: 1) * 60
	[
